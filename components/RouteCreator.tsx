...
const RouteCreator: React.FC<RouteCreatorProps> = ({ unitSystem, onSave, onCancel, initialRoute }) => {
  ...
  const fetchCoffeeShops = async (points: LatLng[]) => {
    ...
    try {
      const query = `[out:json];node["amenity"="cafe"](around:200,${mid.lat},${mid.lng});out;`;
  ...
  const handleDeletePoint = async (id: string) => {
    ...
    if (index === 0) {
      // Deleting start point
      nextSegments.splice(0, 1);
      if (nextSegments.length > 0) {
        // New start point becomes the anchor with no incoming path
      ...
      // Deleting a middle point - bridge the gap
      ...
      nextSegments.splice(index, 1);
      // The segment that was after the deleted one now carries the bridged path from the segment before
  ...
  const handleFinishTrace = async () => {
    ...
    setIsRouting(true);
    const startPoint = tracePointsRef.current[0];
    const endPoint = tracePointsRef.current[tracePointsRef.current.length - 1];
    
    const snappedPath = isSnapRef.current 
      ? await fetchRouteSegment(startPoint, endPoint) 
      : tracePointsRef.current;
    ...
    if (currentSegments.length > 0) {
      const last = currentSegments[currentSegments.length - 1];
      const bridge = await fetchRouteSegment(last.clickPoint, snappedPath[0]);
      const newSeg: RouteSegment = {
        id: newId,
        clickPoint: endPoint,
        pathPoints: [...bridge, ...snappedPath],
        isSnap: isSnapRef.current
      };
      nextSegments = [...currentSegments, newSeg];
    } else {
  ...
  const handleCloseLoop = async () => {
    if (segmentsRef.current.length < 2) return;
    const first = segmentsRef.current[0].clickPoint;
    await handleAddPoint(first);
  };
  ...
  const rebuildMapVisuals = useCallback(() => {
    ...
    currentSegments.forEach((seg, idx) => {
      ...
      if (!marker) {
        ...
        // Bind double click or click to delete
    ...
    cafeMarkersRef.current = cafes.map(cafe => {
      const icon = L.divIcon({
        ...
        html: `<div class="glass" style="width: 38px; height: 38px; border-radius: 14px; display: flex; align-items: center; justify-content: center; font-size: 20px; border: 1.5px solid #d4943a; box-shadow: 0 8px 24px rgba(0,0,0,0.6);">☕</div>`,
        iconSize: [38, 38],
        iconAnchor: [19, 19]
      });
      return L.marker([cafe.lat, cafe.lon], { icon }).addTo(mapInstance.current).bindPopup(`
        <div class="p-2 space-y-1">
          <b class="text-lg">${cafe.tags.name || 'Coffee Pit Stop'}</b>
          <p class="text-xs opacity-60">Barista Approved</p>
        </div>
      `);
    });
  ...
  useEffect(() => {
    ...
    mapInstance.current = L.map(mapRef.current, { zoomControl: false, attributionControl: false }).setView([37.7749, -122.4194], 14);
    ...
    traceLineRef.current = L.polyline([], { color: '#f5e6d0', weight: 3, opacity: 0.4, dashArray: '8, 12' }).addTo(mapInstance.current);
  ...
  return (
    <div className="fixed inset-0 z-[110] bg-[#1a1210] flex flex-col overflow-hidden animate-in fade-in duration-500">
      <div ref={mapRef} className="flex-1 cursor-crosshair" />
      ...
      <div className="absolute top-32 right-8 flex flex-col gap-4 z-[1000]">
         ...
         <button onClick={handleCloseLoop} disabled={segments.length < 2} title="Close Loop" className="glass w-16 h-16 rounded-[1.8rem] flex items-center justify-center text-white disabled:opacity-10 active:scale-90 transition-all">
            <svg className="h-6 w-6" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2.5} d="M4 4v5h.582m15.356 2A8.001 8.001 0 004.582 9m0 0H9m11 11v-5h-.581m0 0a8.003 8.003 0 01-15.357-2m15.357 2H15" /></svg>
         </button>
      ...
      {segments.length === 0 && (
        <div className="absolute top-[30%] left-1/2 -translate-x-1/2 z-[1001] pointer-events-none text-center space-y-4">
           <div className="glass px-10 py-12 rounded-[4rem] border-amber-900/20 shadow-3xl">
              <h3 className="text-4xl font-display font-black text-white leading-none uppercase">Draw your fix</h3>
              <p className="text-[9px] font-bold uppercase tracking-[0.4em] opacity-40 mt-3">Tap map to place pins<br/>Shift + Drag to trace</p>
           </div>
        </div>
      )}
