...
const ClubCreator: React.FC<ClubCreatorProps> = ({ routes, onSave, onCancel }) => {
  ...
  const [logo, setLogo] = useState(`https://images.unsplash.com/photo-1511920170033-f8396924c348?w=400&h=400&fit=crop`);
      ...
      <div className="max-w-xl mx-auto w-full space-y-12 pb-20">
        ...
        <form onSubmit={handleSubmit} className="space-y-10">
          ...
          <div className="space-y-4 flex flex-col items-center">
             <label className="text-[10px] font-black uppercase tracking-[0.3em] opacity-30 block w-full ml-2">Club Profile Picture</label>
             <div className="relative group mb-4">
                <div className="w-32 h-32 rounded-[2.5rem] overflow-hidden border-2 border-[var(--border-color)] group-hover:border-[var(--accent-primary)] transition-all">
                   <img src={logo} className="w-full h-full object-cover" alt="Club Preview" onError={() => setLogo('https://images.unsplash.com/photo-1511920170033-f8396924c348?w=400&h=400&fit=crop')} />
                </div>
                <div className="absolute inset-0 flex items-center justify-center opacity-0 group-hover:opacity-100 transition-opacity bg-black/40 rounded-[2.5rem] pointer-events-none">
             ...
             <input 
              ...
              className="w-full glass bg-white/5 border border-white/10 rounded-2xl px-6 py-4 text-xs font-mono text-[var(--text-muted)] focus:outline-none focus:ring-1 ring-[var(--accent-primary)]/40 transition-all text-center"
             />
          </div>
